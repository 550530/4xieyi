...
# =========================
# 老王sing-box四合一安装脚本 (VLESS明文强改版)
# vless-reality|vless-ws-tls(tunnel)|hysteria2|tuic5
# 最后更新时间: 2026.4
# =========================
...
get_info() {  
  ...
  VLESS_ARGO="vless://${uuid}@${CFIP}:${CFPORT}?encryption=none&security=tls&sni=${argodomain}&fp=firefox&insecure=1&type=ws&host=${argodomain}&path=%2Fvless-argo#${isp}-Argo"
    ...
    case "${choice}" in
        1)
            ...
            case "${choice}" in
                1)
                    ...
                    sed -i '/"tag": "vless-reality"/,/listen_port/ s/"listen_port": [0-9]\+/"listen_port": '"$new_port"'/' $config_dir
                    restart_singbox
                ...
                3)
                    ...
                    ;;
                4)  
                    reading "\n请输入vless-argo端口 (回车跳过将使用随机端口): " new_port
                    [ -z "$new_port" ] && new_port=$(shuf -i 2000-65000 -n 1)
                    sed -i '/"tag": "vless-ws"/,/listen_port/ s/"listen_port": [0-9]\+/"listen_port": '"$new_port"'/' $config_dir
                    allow_port $new_port/tcp > /dev/null 2>&1
                    if command_exists rc-service; then
                    ...
                    restart_singbox
                    green "\nvless-argo端口已修改为：${purple}${new_port}${re}\n"
                    ;;                  
                0)  change_config ;;
                *)  red "无效的选项，请输入 1 到 4" ;;
            esac
        ...
        3)  
            ...
            reading "\n请输入新的Reality伪装域名(可自定义输入,回车留空将使用默认1): " new_sni
                ...
                (.inbounds[] | select(.tag == "vless-reality") | .tls.server_name) = $new_sni |
                (.inbounds[] | select(.tag == "vless-reality") | .tls.reality.handshake.server) = $new_sni
...
# 更新Argo域名到订阅 (已适配vless明文格式)
change_argo_domain() {
    sed -i -E '/vless-argo/s/(sni=)[^&]*/\1'"$ArgoDomain"'/' "$client_dir"
    sed -i -E '/vless-argo/s/(host=)[^&]*/\1'"$ArgoDomain"'/' "$client_dir"
    base64 -w0 ${work_dir}/url.txt > ${work_dir}/sub.txt
    green "vless-argo节点已更新,请更新订阅或手动获取节点\n"
}
...
change_cfip() {
    ...
    sed -i -E '/vless-argo/s/(@)[^:]*(:[0-9]+)/\1'"$cfip"':'"$cfport"'/' "$client_dir"
    base64 -w0 "${work_dir}/url.txt" > "${work_dir}/sub.txt"
    green "\nvless-argo节点优选已更新为：${purple}${cfip}:${cfport}${re}\n"
}
...
menu() {
   ...
   purple "=== 老王sing-box四合一安装脚本 (VLESS明文版) ===\n"
